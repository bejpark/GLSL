...
void startup()
{
	...
	mat4 projection = perspective(radians(60.0f), (float)sc.width/sc.height, 0.3f, 100.0f);
	...
	glUniformMatrix4fv(sc.proj_loc, 1, GL_FALSE, value_ptr(projection));
...
void initBuffers()
{
	...
	for (unsigned int i = 0; i < sc.nParticles; i++) {
		...
		velocity = mix(1.25f,8.5f, randFloat());
