...
void startup()
{
	...
	sc.proj = perspective(radians(70.0f), (float)(WIDTH / HEIGHT), 0.3f, 100.0f);
...
void render()
{
	...
	// Matrix
	sc.model = translate(mat4(1.0f), vec3(2.0, 0.0, -2.0)); 
	...
	sc.model = translate(mat4(1.0f), vec3(0.0, 1.0, 3.0));
	sc.model = rotate(sc.model, (float)radians(-90.0), vec3(1.0, 0.0, 0.0));
	glUniformMatrix4fv(sc.m_loc, 1, GL_FALSE, value_ptr(sc.model));
	sc.torus->render();
